...
export class LoginPage {
  ...
  readonly showPassword: Locator = this.root.locator('button[type="button"]');
  readonly invalidUsernameOrPassword: Locator = this.root.locator('text=Invalid username or password, Please try again');
  readonly home: Locator = this.root.locator('a[href="/"]');
  ...
  async showPasswordClick(): Promise<void> {
    await this.showPassword.click();
  }

  async verifyLoginVisibleElements(): Promise<void> {
    await this.username.waitFor({ state: 'visible' });
    await this.password.waitFor({ state: 'visible' });
    await this.login.waitFor({ state: 'visible' });
    await this.showPassword.waitFor({ state: 'visible' });
